...
``rapidsms.contrib.scheduler`` is currently incompatible with v0.10.0. Until we release a compatible version, we recomend investigating cron-style methods or using Celery's `periodic tasks`_.
