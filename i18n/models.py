...
class LanguageManager(models.Manager):
    def get_query_set(self):
        ...
        for app in PersistantApp.objects.all():
            try:
                for lang_code in app.locale.keys():
...
class PersistantApp(models.Model):
    ...
    def locale(self):
        ...
        locale_module = __import__("%s.locale" % (self.module), {}, {}, ["LOCALE"])
        return getattr(locale_module, "LOCALE")
...
class String(models.Model):
    ...
    def resolve(cls, lang, app, token):
        """Returns a translated string, ready to be formatted and returned to
           ...
           possible), like so:
            ...
            Anyhoo, this method checks, in order:
              ...
              [5] the token itself, wrapped in braces to indicate that
                  something is awry.
        ...
        # resolve the app into an object,
        # if it isn't already, to allow a
        # string to be passed instead
        if not isinstance(app, PersistantApp):
            try:
                app_module = app.config["module"] if isinstance(app, rapidsms.App) else app
                app = PersistantApp.objects.get(module=app_module)

            # if the requested doesn't exist, wrap the exception in
            # something more useful that callers can look out for
            # without knowledge of the internals of this class
            except PersistantApp.DoesNotExist, err:
                raise(cls.UnknownApp(err))
        ...
        # look in the app's locale.py next, for the default
            ...
            try:
                str = app.locale[lang.code][token.slug]
