...
class EventSchedule(models.Model):
    ...
    months = PickledObjectField(null=True, blank=True,
        help_text="'1,2,3' for jan, feb, march - '*' for all")
    days_of_month = PickledObjectField(null=True, blank=True,
        help_text="'1,2,3' for 1st, 2nd, 3rd - '*' for all")
    days_of_week = PickledObjectField(null=True, blank=True,
        help_text="'0,1,2' for mon, tue, wed - '*' for all")
    hours = PickledObjectField(null=True, blank=True,
        help_text="'0,1,2' for midnight, 1 o'clock, 2 - '*' for all")
    minutes = PickledObjectField(null=True, blank=True,
        help_text="'0,1,2' for X:00, X:01, X:02 - '*' for all")
    ...
    count = models.IntegerField(null=True, blank=True,
        help_text="How many times do you want this to fire? Leave blank for" +
        "'continuously'")
    ...
    def __unicode__(self):
        def _set_to_string(set, conversion_dict=None):
            if len(set) > 0:
                if conversion_dict is not None:
                    return ", ".join(
                        [unicode(conversion_dict[m]) for m in set])
                else:
        ...
        return "%s: Months:(%s), Days of Month:(%s), Days of Week:(%s), " +\
            "Hours:(%s), Minutes:(%s)" % \
            (self.callback, months, days_of_month, days_of_week, hours,
                minutes)
    ...
    def validate(months, days_of_month, days_of_week, hours, minutes):
        ...
        i.e. when a weekly schedule is set, one must also specify day, hour,
        and minute.
        """
        EventSchedule.validate_ranges(months, days_of_month, days_of_week,
            hours, minutes)
        EventSchedule.validate_subtimespans(months, days_of_month,
            days_of_week, hours, minutes)
    ...
    @staticmethod
    def validate_subtimespans(months, days_of_month, days_of_week, hours,
                                minutes):
        if len(minutes) == 0 and len(hours) == 0 and len(days_of_week) == 0\
            and len(days_of_month) == 0 and len(months) == 0:
        ...
        if len(days_of_month) > 0 and len(hours) == 0:
            raise EventSchedule.UndefinedSchedule("Must specify hour(s)")
        if len(months) > 0 and len(days_of_month) == 0 and\
            len(days_of_week) == 0:
    ...
    def save(self, *args, **kwargs):
        ...
        in the ui, saving it, editing it, saving it, editing it continues to
        work with callback_args, kwargs, and different timespans
    ...
    def should_fire(self, when):
        """
        Return True if this event should trigger at the specifieddatetime
        """
        if self.start_time:
        ...
        # pickle well. This would be alleviated if this functionality were
        # optimized to stop doing db calls on every fire
        ...
        # the following ensures that 'days of month' will override empty
        # 'day of week' and vice versa
