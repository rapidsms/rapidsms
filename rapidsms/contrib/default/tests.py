...
from rapidsms.tests.harness import EchoApp, RapidTest, setting
...
class DefaultAppTest(RapidTest):
    ...
    def test_no_handle(self):
        ...
        self.set_router()
        with setting(RAPIDSMS_ROUTER=self.router_class,
    ...
    def test_response_is_none(self):
        ...
        with setting(DEFAULT_RESPONSE=None):
    ...
    def test_response_no_project_name(self):
        ...
        with setting(DEFAULT_RESPONSE='hello'):
            self.receive('asdf', self.connection)
            self.assertEqual(len(self.outbound), 1)
    ...
    def test_response_with_project_name(self):
        ...
        with setting(DEFAULT_RESPONSE='hello %(project_name)s',
                     PROJECT_NAME='world'):
            self.receive('asdf', self.connection)
            self.assertEqual(len(self.outbound), 1)
