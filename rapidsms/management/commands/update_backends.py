...
class Command(BaseCommand):
    help = (
        "Creates an instance of the Backend model stub for each " + "running backend."
    )
    ...
    def handle(self, **options):
        ...
        known_backend_names = list(Backend.objects.values_list("name", flat=True))
        ...
        for name in settings.INSTALLED_BACKENDS:
            if name not in known_backend_names:
                ...
                backend = Backend.objects.create(name=name)
