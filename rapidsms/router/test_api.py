...
class RouterAPITest(harness.RapidTest):
    ...
    def test_send_with_connection(self):
        ...
        self.assertEqual(message.connections[0].backend.name, connection.backend.name)
    ...
    def test_send_with_connections(self):
        ...
        self.assertEqual(message.connections[0].identity, connections[0].identity)
        self.assertEqual(
            message.connections[0].backend.name, connections[0].backend.name
        )
    ...
    def test_saved_message_fields_receive(self):
        ...
        message = receive("test incoming message", connection=connection, fields=fields)
    ...
    def test_receive_message_class(self):
        ...
        msg = self.receive(
            "echo hello", self.create_connection(), class_=TestIncomingMessage
        )
    ...
    def test_send_message_class(self):
        ...
        msg = self.send("hello", self.create_connection(), class_=TestIncomingMessage)
        self.assertTrue(isinstance(msg, TestIncomingMessage))
