...
from rapidsms.tests.harness.backend import FailedIdentitiesBackend, RaisesBackend
...
class MessageStatusTest(harness.CustomRouterMixin, TestCase):
    ...
    def test_outbound_message_status_error(self):
        ...
        dbm.transmissions.create(connection=self.create_connection(), status="E")
        dbm.transmissions.create(connection=self.create_connection(), status="S")
        self.assertEqual("E", dbm.set_status())
    ...
    def test_outbound_message_status_sent(self):
        ...
        dbm = Message.objects.create(text="test", direction="O")
        dbm.transmissions.create(connection=self.create_connection(), status="S")
        dbm.transmissions.create(connection=self.create_connection(), status="D")
...
class DatabaseRouterReceiveTest(harness.DatabaseBackendMixin, TestCase):
    ...
    def setUp(self):
        self.conn1 = self.lookup_connections(
            backend="mockbackend", identities=["5551212"]
        )[0]
        self.conn2 = self.lookup_connections(
            backend="mockbackend", identities=["5552323"]
        )[0]
    ...
    def test_create_message_from_dbm(self):
        ...
        router = DatabaseRouter()
        dbm = router.queue_message(direction="I", text="foo", connections=[self.conn1])
    ...
    def test_in_response_to_external_id(self):
        ...
        connection = self.lookup_connections(["1112223333"])[0]
        msg = self.receive("test", connection, fields={"external_id": "ABCD1234"})
    ...
    def test_receive_async_fields(self):
        ...
        mock_method.assert_called_once_with(message_id=msg.id, fields=msg.fields)
...
class DatabaseRouterSendTest(harness.DatabaseBackendMixin, TestCase):
    ...
    def create_many_transmissions(self, num, backend=None):
        ...
        for i in range(num):
            connection = Connection.objects.create(identity="%07d" % i, backend=backend)
    ...
    def test_group_transmissions(self):
        ...
        router = DatabaseRouter()
        trans = list(
            router.group_transmissions(Transmission.objects.all(), batch_size=2)
        )
        _, batch1 = trans[0]
        _, batch2 = trans[1]
        self.assertEqual(
            list(batch1.values_list("id", flat=True)),
            list(t1.values_list("id", flat=True)),
        )
        self.assertEqual(
            list(batch2.values_list("id", flat=True)),
    ...
    def test_group_transmissions_different_backends(self):
        ...
        router = DatabaseRouter()
        result = list(
            router.group_transmissions(Transmission.objects.all(), batch_size=4)
        )
        # 4 total batches
    ...
    def test_all_transmissions_set_to_E_if_backend_sending_error(self):
        ...
        with override_settings(INSTALLED_BACKENDS=self.backends):
            with self.assertRaises(MessageSendingError):
                send_transmissions(
                    error_backend.model.pk, dbm.pk, both_transmisssion_sets
                )
