...
class Message(models.Model):
    ...
    status = models.CharField(
        max_length=1, choices=STATUS_CHOICES, default="Q", db_index=True
    )
...
class Transmission(models.Model):
    ...
    message = models.ForeignKey(
        Message, related_name="transmissions", on_delete=models.CASCADE
    )
    #: Required. Foreign key to associated ``Connection``.
    connection = models.ForeignKey(
        Connection, related_name="transmissions", on_delete=models.CASCADE
    )
    #: Required. See :ref:`message-status-values`.
