...
urlpatterns = (
    ...
    path(
        "backend/http-custom/", CustomHttpBackend.as_view(), name="custom-http-backend"
    ),
)
...
class HttpFormTest(TestCase):
    ...
    def test_get_incoming_data(self):
        ...
        self.assertEqual(data["identity"], incoming_data["connection"].identity)
        self.assertEqual("http-backend", incoming_data["connection"].backend.name)
...
class HttpViewTest(RapidTest):
    ...
    def test_valid_post_message(self):
        ...
        self.assertEqual(data["identity"], message.connections[0].identity)
        self.assertEqual("http-backend", message.connections[0].backend.name)
    ...
    def test_valid_post_message_backend_name(self):
        ...
        message = self.inbound[0]
        self.assertEqual("custom-http-backend", message.connections[0].backend.name)
