...
from rapidsms.tests.harness import CreateDataMixin, RapidTest
...
class VumiFormTest(TestCase):
    ...
    def test_get_incoming_data(self):
        ...
        self.assertEqual(
            self.valid_data["from_addr"], incoming_data["connection"].identity
        )
        self.assertEqual("vumi-backend", incoming_data["connection"].backend.name)
...
class VumiViewTest(RapidTest):
    ...
    def test_invalid_response(self):
        ...
        response = self.client.post(
            reverse("vumi-backend"), json.dumps(data), content_type="text/json"
        )
    ...
    def test_invalid_json(self):
        ...
        data = "{bad json, , lala}"
        response = self.client.post(
            reverse("vumi-backend"), data, content_type="text/json"
        )
        self.assertEqual(response.status_code, 400)
    ...
    def test_valid_post_message(self):
        ...
        self.assertEqual(self.valid_data["from_addr"], message.connections[0].identity)
        self.assertEqual("vumi-backend", message.connections[0].backend.name)
    ...
    def test_blank_message_is_valid(self):
        ...
        for blank_msg in [empty, null, no_content]:
            self.client.post(
                reverse("vumi-backend"), json.dumps(blank_msg), content_type="text/json"
            )
            message = self.inbound[0]
...
class VumiSendTest(CreateDataMixin, TestCase):
    ...
    def test_outgoing_keys(self):
        ...
        kwargs = backend.prepare_request(
            message.id, message.text, [message.connections[0].identity], {}
        )
    ...
    def test_bulk_response_external_id(self):
        ...
        backend = VumiBackend(None, "vumi", **config)
        kwargs = backend.prepare_request(
            "1234", "foo", [conn1.identity, conn2.identity], {"external_id": "ASDF1234"}
        )
        data = json.loads(kwargs["data"])
    ...
    def test_from_addr_and_endpoint_in_payload(self):
        ...
        config = {"sendsms_url": "http://example.com"}
        backend = VumiBackend(None, "vumi", **config)
        context = {"from_addr": "5551212", "endpoint": "12345", "other": "not included"}
        kwargs = backend.prepare_request(
            message.id, message.text, [message.connections[0].identity], context
        )
        data = json.loads(kwargs["data"])
