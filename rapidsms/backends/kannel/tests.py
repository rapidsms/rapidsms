...
class KannelViewTest(RapidTest):
    ...
    def test_valid_post_message(self):
        ...
        self.assertEqual(data["id"], message.connections[0].identity)
        self.assertEqual("kannel-backend", message.connections[0].backend.name)
...
class KannelSendTest(CreateDataMixin, TestCase):
    def test_outgoing_keys(self):
        ...
        kwargs = backend.prepare_request(
            1, message.text, [message.connections[0].identity], {}
        )
        data = kwargs["params"]
        self.assertEqual(config["sendsms_params"]["smsc"], data["smsc"])
        self.assertEqual(config["sendsms_params"]["from"], data["from"])
        self.assertEqual(config["sendsms_params"]["username"], data["username"])
        self.assertEqual(config["sendsms_params"]["password"], data["password"])
